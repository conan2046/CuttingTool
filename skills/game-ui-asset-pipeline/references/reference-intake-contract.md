...
首次创建项目目录后先等待用户放置参考图。参考图是后续风格基准和生产 Sheet 的视觉来源，输入不合格会把问题放大到整批资源，因此在资源清单和生成前设置强制验收关卡。
...
通过后才进入资源清单、请求 JSON、Layout Guide 和图片生成。
