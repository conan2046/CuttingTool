...
3. 自动创建的 `reference-notes.md` 已包含填写注释、可见示例和占位表格。你可以照例补充参考图职责。
4. Skill 告知参考图目录后会暂停。你把参考图放进 `references\`，然后回复“已放好”。
5. Skill 先检查文件格式、尺寸、命名和重复内容，再逐张查看清晰度、完整性、职责和风格冲突。
6. 不合格时按提示替换图片并再次回复；全部通过后才继续填写和处理资源清单。
...
当前 JSON 中辅助图按 `references` 列表顺序传入，统一标记为辅助风格参考。因此必须在自然语言需求的“参考图作用”中写清楚每张图负责什么。
