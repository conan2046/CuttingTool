...
第一次使用请直接阅读 [新手操作指南](BEGINNER_GUIDE.md)。Skill 会先取得项目名，再自动创建 `input/<项目名>/references/reference-notes.md`；该目录中的私人素材默认不会提交到 Git。

## 当前能力

- 创建资源任务运行目录。
- 首次使用时询问缺失的项目名，并自动初始化本地参考图目录和说明文件；重复调用不覆盖用户内容。
- 初始化后暂停等待用户放图；用户确认后执行文件级与逐图视觉验收，不合格时阻断资源清单和生成。
- 同一请求编排多个资源分类。
